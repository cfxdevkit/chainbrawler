...
abstract contract CombatEngine is ChainBrawlerState {
    ...
    function _getEnemyBase(uint256 id) internal view virtual returns (
        uint256 baseCombat, 
    ...
    function getScaledEnemyStatsInternal(
        ...
        (enemyCombat, enemyEndurance, enemyDefense, enemyLuck) = 
            CombatMath.scaleEnemyForLevel(baseCombat, baseEndurance, baseDefense, baseLuck, enemyLevel);
    ...
    function _fightEnemyInternalWithLevel(uint256 enemyId, uint256 enemyLevel, bool allowExtraRound) internal virtual {
        ...
        (uint256 enemyCombat, uint256 enemyEndurance, uint256 enemyDefense, uint256 enemyLuck) = 
            getScaledEnemyStatsInternal(enemyId, enemyLevel);

        CombatResult memory result = CombatEngineLib.executeCombat(
            msg.sender, enemyId, enemyLevel, allowExtraRound, packedCharacters, combatStates,
            baseCombat, enemyCombat, enemyEndurance, enemyDefense, enemyLuck
        );
    ...
    function _emitUnresolvedCombat(CombatResult memory result) internal {
        emit FightSummary(
            msg.sender, result.enemyId, result.enemyLevel, result.roundsElapsed,
            result.playerStartEndurance, result.currentEndurance,
            result.enemyStartEndurance, result.enemyCurrentEndurance,
            false, true, result.roundNumbers, result.playerDamages,
            result.enemyDamages, result.playerCriticals, result.enemyCriticals
        );
    }
    ...
    function _handleVictory(CombatResult memory result, uint256 enemyId, uint256 enemyLevel) internal returns (uint256 xpGained) {
        uint256 totalKills = (result.progressionStats >> BitPackedCharacterLib.TOTAL_KILLS_SHIFT) & BitPackedCharacterLib.TOTAL_KILLS_MASK;
        result.progressionStats = SafePacker.writeClamped(
            result.progressionStats, BitPackedCharacterLib.TOTAL_KILLS_SHIFT, 
            BitPackedCharacterLib.TOTAL_KILLS_MASK, totalKills + 1
        );
    ...
    function _processLevelUp(CombatResult memory result, uint256 xpGained) internal returns (CombatResult memory) {
        uint256 experience = (result.progressionStats >> BitPackedCharacterLib.EXPERIENCE_SHIFT) & BitPackedCharacterLib.EXPERIENCE_MASK;
        experience += xpGained;
        
        (result.coreStats, experience) = checkLevelUpAndAdjustXP(msg.sender, experience, result.coreStats);
        result.progressionStats = SafePacker.writeClamped(
            result.progressionStats, BitPackedCharacterLib.EXPERIENCE_SHIFT, 
            BitPackedCharacterLib.EXPERIENCE_MASK, experience
        );
    ...
    function _handleDefeat(CombatResult memory result) internal returns (uint256) {
        ...
        result.coreStats = SafePacker.writeClamped(
            result.coreStats, BitPackedCharacterLib.IS_ALIVE_SHIFT, 
            BitPackedCharacterLib.IS_ALIVE_MASK, 0
        );
    ...
    function _resetEquipment(uint256 coreStats) internal pure returns (uint256) {
        coreStats = SafePacker.writeClamped(coreStats, BitPackedCharacterLib.EQUIPPED_COMBAT_SHIFT, 
            BitPackedCharacterLib.EQUIPPED_COMBAT_MASK, 0);
        coreStats = SafePacker.writeClamped(coreStats, BitPackedCharacterLib.EQUIPPED_ENDURANCE_SHIFT, 
            BitPackedCharacterLib.EQUIPPED_ENDURANCE_MASK, 0);
        coreStats = SafePacker.writeClamped(coreStats, BitPackedCharacterLib.EQUIPPED_DEFENSE_SHIFT, 
            BitPackedCharacterLib.EQUIPPED_DEFENSE_MASK, 0);
        coreStats = SafePacker.writeClamped(coreStats, BitPackedCharacterLib.EQUIPPED_LUCK_SHIFT, 
            BitPackedCharacterLib.EQUIPPED_LUCK_MASK, 0);
        return coreStats;
    }
    ...
    function _updateRegenTime(CombatResult memory result) internal {
        if (result.lastEnduranceChanged) {
            result.progressionStats = SafePacker.writeClamped(
                result.progressionStats, BitPackedCharacterLib.LAST_REGEN_TIME_SHIFT, 
                BitPackedCharacterLib.LAST_REGEN_TIME_MASK, result.timestamp
            );
            packedCharacters[msg.sender].progressionStats = result.progressionStats;
        }
    }
    ...
    function _emitFightSummary(CombatResult memory result, uint256 /*xpGained*/) internal {
        emit FightSummary(
            ...
            result.playerStartEndurance, result.currentEndurance,
            result.enemyStartEndurance, result.enemyCurrentEndurance,
            result.unresolved ? false : result.victory, result.unresolved,
            result.roundNumbers, result.playerDamages, result.enemyDamages,
            result.playerCriticals, result.enemyCriticals
        );
    }
    ...
    function _validatePlayerAlive(CombatState memory cs, BitPackedCharacterLib.BitPackedCharacter storage packedChar) internal returns (bool) {
        ...
        if (!actualIsAlive || actualCurrentHP == 0) {
            ...
            packedChar.coreStats = SafePacker.writeClamped(
                packedChar.coreStats, BitPackedCharacterLib.IN_COMBAT_SHIFT, 
                BitPackedCharacterLib.IN_COMBAT_MASK, 0
            );
            
            emit FightSummary(
                msg.sender, cs.enemyId, cs.enemyLevel, cs.roundsElapsed,
                cs.playerStartEndurance, 0, cs.enemyStartEndurance, cs.enemyCurrentEndurance,
                false, false, new uint256[](0), new uint256[](0), new uint256[](0),
                new bool[](0), new bool[](0)
            );
            return false;
        }
        return true;
    ...
    function _updateCombatStateHP(CombatState memory cs, BitPackedCharacterLib.BitPackedCharacter storage packedChar) 
    ...
    function _executeFleeRound(CombatState memory cs, BitPackedCharacterLib.BitPackedCharacter storage packedChar) 
        ...
        uint256 combatSkill = ((fleeData.coreStats >> BitPackedCharacterLib.COMBAT_SKILL_SHIFT) & BitPackedCharacterLib.COMBAT_SKILL_MASK)
            + ((fleeData.coreStats >> BitPackedCharacterLib.EQUIPPED_COMBAT_SHIFT) & BitPackedCharacterLib.EQUIPPED_COMBAT_MASK);
        uint256 defense = ((fleeData.coreStats >> BitPackedCharacterLib.DEFENSE_SHIFT) & BitPackedCharacterLib.DEFENSE_MASK)
            + ((fleeData.coreStats >> BitPackedCharacterLib.EQUIPPED_DEFENSE_SHIFT) & BitPackedCharacterLib.EQUIPPED_DEFENSE_MASK);
        uint256 playerLuck = ((fleeData.coreStats >> BitPackedCharacterLib.LUCK_SHIFT) & BitPackedCharacterLib.LUCK_MASK)
            + ((fleeData.coreStats >> BitPackedCharacterLib.EQUIPPED_LUCK_SHIFT) & BitPackedCharacterLib.EQUIPPED_LUCK_MASK);

        (uint256 enemyCombat, , uint256 enemyDefense, uint256 enemyLuck) = getScaledEnemyStatsInternal(cs.enemyId, cs.enemyLevel);

        (fleeData.playerAfter, fleeData.enemyAfter, fleeData.playerDamage, fleeData.enemyDamage, , 
         fleeData.playerCritical, fleeData.enemyCritical) = CombatMath.performRound(
            combatSkill, enemyCombat, enemyDefense, defense,
            cs.playerCurrentEndurance, cs.enemyCurrentEndurance,
            playerLuck, enemyLuck
        );

        fleeData.coreStats = BitPackedCharacterLib.setCurrentEnduranceSafe(fleeData.coreStats, fleeData.playerAfter);
        fleeData.coreStats = SafePacker.writeClamped(fleeData.coreStats, BitPackedCharacterLib.IS_ALIVE_SHIFT, 
            BitPackedCharacterLib.IS_ALIVE_MASK, fleeData.playerAfter > 0 ? 1 : 0);
    }
    ...
    function _handleFleeDeath(CombatState memory cs, FleeRoundData memory fleeData, 
        BitPackedCharacterLib.BitPackedCharacter storage packedChar) internal {
        fleeData.coreStats = (fleeData.coreStats & ~(BitPackedCharacterLib.IS_ALIVE_MASK << BitPackedCharacterLib.IS_ALIVE_SHIFT));
        fleeData.coreStats = SafePacker.writeClamped(fleeData.coreStats, BitPackedCharacterLib.IN_COMBAT_SHIFT, 
            BitPackedCharacterLib.IN_COMBAT_MASK, 0);
    ...
    /// @param fleeData The flee round data
    /// @param packedChar The packed character data
    function _handleFleeVictory(CombatState memory cs, FleeRoundData memory fleeData, 
        BitPackedCharacterLib.BitPackedCharacter storage packedChar) internal {
        (, , , , uint256 xpGainedRaw, ) = _getEnemyBase(cs.enemyId);
        uint256 xpGained = fleeData.level < 4 ? (xpGainedRaw * 150) / 100 : xpGainedRaw;
        
        uint256 experience = (fleeData.coreStats >> BitPackedCharacterLib.EXPERIENCE_SHIFT) & BitPackedCharacterLib.EXPERIENCE_MASK;
        ...
        uint256 totalKills = (fleeData.progressionStats >> BitPackedCharacterLib.TOTAL_KILLS_SHIFT) & BitPackedCharacterLib.TOTAL_KILLS_MASK;
        fleeData.progressionStats = SafePacker.writeClamped(fleeData.progressionStats, BitPackedCharacterLib.TOTAL_KILLS_SHIFT, 
            BitPackedCharacterLib.TOTAL_KILLS_MASK, totalKills + 1);
    ...
    /// @param fleeData The flee round data
    /// @param packedChar The packed character data
    function _handleFleeEscape(CombatState memory cs, FleeRoundData memory fleeData, 
        BitPackedCharacterLib.BitPackedCharacter storage packedChar) internal {
        fleeData.coreStats = SafePacker.writeClamped(fleeData.coreStats, BitPackedCharacterLib.IN_COMBAT_SHIFT, 
            BitPackedCharacterLib.IN_COMBAT_MASK, 0);
        packedChar.coreStats = fleeData.coreStats;
    ...
    function _emitFleeRoundSummary(CombatState memory cs, FleeRoundData memory fleeData, bool victory, bool unresolved) internal {
        ...
        emit FightSummary(
            msg.sender, cs.enemyId, cs.enemyLevel, cs.roundsElapsed + 1,
            cs.playerCurrentEndurance, fleeData.playerAfter,
            cs.enemyCurrentEndurance, fleeData.enemyAfter,
            unresolved ? false : victory, unresolved,
            roundNumbers, playerDamages, enemyDamages,
            playerCriticals, enemyCriticals
        );
    }
}
