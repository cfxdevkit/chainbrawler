...
describe('ReactAdapter', () => {
  ...
  it('should delegate getState to SDK', () => {
    ...
    mockSdk.getState.mockReturnValue(mockState);

    const state = adapter.getState();
    
    expect(mockSdk.getState).toHaveBeenCalled();
  ...
  it('should delegate clearError to SDK', () => {
    adapter.clearError();
    expect(mockSdk.clearError).toHaveBeenCalled();
  });
  ...
  it('should store player address from config', () => {
  ...
  it('should handle claimPrize with different parameter types', async () => {
    ...
    expect(mockSdk.actions.claimPrize).toHaveBeenCalledWith(10n, 5n, 2500n, ['0xproof1', '0xproof2']);
  });
