...
import { UXStore, ChainBrawlerConfig, UXState, ChainBrawlerSDK, ContractClientFactory, EventType } from '@chainbrawler/core';
...
export class WebAdapter {
  ...
  constructor(config: ChainBrawlerConfig) {
    ...
    if (walletClient?.account?.address) {
      console.log('WebAdapter: Constructor - setting player address:', walletClient.account.address);
  ...
  async updateWalletClient(walletClient: WalletClient | undefined): Promise<void> {
    ...
    if (walletClient?.account?.address) {
      console.log('WebAdapter: Setting player address and loading character data for:', walletClient.account.address);
      this.sdk.getSDK().setPlayerAddress(walletClient.account.address);
