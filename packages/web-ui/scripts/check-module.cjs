...
async function checkModule() {
  ...
  const checkReactUIModule = () => {
    return new Promise((resolve, reject) => {
      const req = http.get('http://localhost:3000/@fs/workspaces/chainbrawler_dev/packages/react-ui/dist/index.js', (res) => {
        let data = '';
  ...
  try {
    ...
    if (hasFormatDamage) {
      ...
      const formatDamageMatch = moduleResult.content.match(/function formatDamage\([^)]*\)[^{]*\{[^}]*\}/g);
