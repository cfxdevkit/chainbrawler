...
export function calculateEnemyStats(enemyId: number, level: number): {
  combat: number;
  defense: number;
  luck: number;
  health: number;
  ...
  return {
    combat: Math.floor(config.baseStats.combat * Math.pow(config.levelMultipliers.combat, level - 1)),
    defense: Math.floor(config.baseStats.defense * Math.pow(config.levelMultipliers.defense, level - 1)),
    luck: Math.floor(config.baseStats.luck * Math.pow(config.levelMultipliers.luck, level - 1)),
    health: Math.floor(50 * Math.pow(config.levelMultipliers.health, level - 1))
  };
}

export function calculateEnemyRewards(enemyId: number, level: number): {
  xp: number;
  ...
  return {
    xp: Math.floor(config.rewards.baseXP * Math.pow(config.rewards.levelMultiplier, level - 1)),
