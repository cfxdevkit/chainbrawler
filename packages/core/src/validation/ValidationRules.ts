...
export class ValidationRules {
  ...
  static canStartOperation(type: string, state: UXState): ValidationResult {
    ...
    switch (type) {
      case 'createCharacter':
        return this.canCreateCharacter(state);
      case 'fightEnemy':
        return this.canFight(state);
      case 'healCharacter':
        return this.canHeal(state);
      case 'resurrectCharacter':
        return this.canResurrect(state);
      case 'continueFight':
        return this.canContinueFight(state);
      case 'fleeRound':
        return this.canFlee(state);
      default:
        return { valid: true };
    }
  }
  ...
  static validateCharacterState(character: CharacterData | null): ValidationResult {
    ...
    if (character.exists && character.endurance.percentage < 0 || character.endurance.percentage > 100) {
